...
Buoy releases on GitHub only. It is not published to PyPI. The annotated `v0.2.0` tag is preserved as a failed pre-release attempt without a GitHub Release, `v0.2.1` is the first published Release, and `v0.3.0` is the latest published Release. The next planned release is `v0.4.0`.
...
3. Run the exact portable checks used by CI:
   ...
   uv run --no-project python scripts/release_checks.py tag --tag v0.4.0
...
git switch main
git pull --ff-only
git tag -a v0.4.0 -m "Buoy v0.4.0"
...
- download and inspect both `buoy_search-0.4.0-py3-none-any.whl` and `buoy_search-0.4.0.tar.gz`;
