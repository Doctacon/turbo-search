...
## Draft authority and activation gate

This is a **draft focused specification for confirm-or-correct review only**. It proposes source-only post-line-range subdivision for the two active Python-aware treatment arms. It does not change `.10x/specs/repo-python-syntax-chunking-experiment.md`, authorize implementation or tests, regenerate a plan or forecast, or make C6 executable.
...
| Incompatible row class | Exact plan rows | Unique `(repository, row_id)` values | Distinct report `repo_path` strings | Proposed disposition |
|---|---:|---:|---:|---|
| Treatment source with exact physical-line ranges | 20,926 | 11,427 | 4,105 | Candidate source-line subdivision below |
| Unchanged prose path | 366 | 183 | 57 | Remains fail-closed and unresolved; this draft MUST NOT invent prose subdivision |

The two row classes have disjoint `repo_path` strings, producing the checkpoint's exact 4,162-path-string union. Source rows represent 4,111 repository-qualified paths because six source path strings occur in more than one repository; the preserved report intentionally defines its headline path count from the unqualified `repo_path` field. The 366 prose plan rows are 9 pytest rows per treatment arm and 174 Ruff rows per treatment arm. Therefore this source-only proposal cannot by itself make C6 tokenizer-ready.

## Provenance map

| Proposed or observed semantic | Provenance before ratification |
|---|---|
...
| Treatment-only, physical-source-line boundaries, maximal contiguous subranges, exact citations/reconstruction, stable ordering, unchanged control/header | User-directed shaping constraints; not active behavior |
| Preserve each parent range's exact breadcrumb tuple on every child and never cross an existing parent boundary | Candidate mechanical composition; requires confirm-or-correct ratification |
| Farthest-feasible exhaustive prefix algorithm rather than binary search or approximate counts | Candidate deterministic algorithm; requires confirm-or-correct ratification |
| A one-line rendered candidate above 512 invalidates its complete repository/arm plan | Candidate fail-closed scope; requires confirm-or-correct ratification |
...
No record hardening or review can activate the candidate semantics. Activation requires independent review followed by explicit user ratification and supersession of the active syntax contract.

## Purpose and scope

If ratified later, the behavior would add one deterministic compatibility pass after the active syntax processor has produced its exact treatment `SourceRange` values and before treatment `MarkdownChunk` values are finalized. It would apply only to source ranges in:

- `fixed-80-python-breadcrumbs`; and
- `python-ast`, including its Python-parse and non-Python fixed/no-breadcrumb fallbacks.

It would not process `current-default`, common repository header chunks, Markdown/prose chunks, or ordinary no-arm/metadata/card paths. It would not rerun AST ownership, decorator matching, trivia attachment, fallback classification, or the generic Markdown/sentence splitter.

## Exact token accounting

### Pinned tokenizer

Every compatibility decision would use only the already-pinned offline tokenizer:
...
A missing/mismatched revision, implementation, file identity, package lock, maximum, or option would fail local planning. An approximate token count could be retained only for descriptive reporting; it could not make a boundary or readiness decision.
...
Copying the parent tuple is the smallest post-line-range composition and makes overhead deterministic. Recomputing narrower fixed-window breadcrumb sets would be a different treatment semantic and is not implied by this draft.
...
An individually unsplittable physical line is a line whose exact one-line candidate, including its inherited breadcrumbs, final citation/path/title, fence/language, separators, and special tokens, exceeds 512. Its handling would be:
...
Only after independent review, explicit ratification, active-spec supersession, and a bounded executable implementation ticket could source/tests change. That later work would have to prove at least:
...
### Recommended candidate: physical-line exhaustive farthest prefix
