...
- **Exact model-tokenizer readiness fails:** offline tokenizer-only preflight used `BAAI/bge-small-en-v1.5@5c38ec7c405ec4b44b94cc5a9bb96e735b38267a`, exact tokenizer file identity `9c7beccadaa552c323907a895ad9ab188d8b75763022403f72c5d91085334f3b`, and its 512-token maximum with special tokens, no truncation, and no model construction/inference. Across 91,214 treatment plan rows it found 21,292 incompatible rows on 4,162 paths, maximum 12,785 tokens. Exact row IDs, paths, section paths, and counts are recorded in `.10x/evidence/.storage/2026-07-20-c6-python-syntax-tokenizer-preflight.json.gz` at checkpoint `c3a1560e611114760909c110a118a3ce1a60f0527de08c769a85a20b263f4e0f`. Read-only shaping decomposed that total into exactly 20,926 treatment source rows eligible for candidate physical-line subdivision and 366 unchanged prose plan rows (183 unique repository/row identities across 57 paths, duplicated across the two treatments). Draft `.10x/specs/deterministic-treatment-token-budget-subdivision.md` is inactive pending independent review and explicit ratification; it does not define prose subdivision. Therefore every current incompatible row still fails readiness, and the 366 prose rows remain an independent fail-closed stop even if source subdivision is later ratified and implemented. Ruff additionally retains 559/2,875 Python parse fallbacks per treatment; deterministic fallback path-category hashes are in the compact forecast.
- **Independent review and write approval remain absent:** repaired forecast records and deterministic validation require independent review at the refreshed PR head. No exact approval exists for any of the nine namespace writes, and failed exact tokenizer readiness prevents presenting the unchanged write-approval checkpoint as passing. Separate shaping grants no C6 implementation or write authority. C6 remains `blocked`; no apply, retrieval, catalog/state/default change, delete, or promotion is authorized.
...
- `.10x/specs/deterministic-treatment-token-budget-subdivision.md` (draft; non-authoritative)
...
- 2026-07-20: Source-only shaping produced an inactive focused draft and exact checkpoint decomposition: 20,926 incompatible source plan rows and 366 incompatible unchanged prose plan rows sum to the preserved 21,292. The draft proposes no prose behavior, so those 366 rows remain a separate fail-closed readiness stop. Even after any future source-contract ratification, C6 cannot become approval-ready until prose behavior receives separate authority and a complete regenerated exact preflight has zero incompatible rows. No active spec, source/test, plan/forecast/token report, validator, namespace, or live state changed.
